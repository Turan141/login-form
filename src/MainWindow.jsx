...
function MainWindow() {
  ...
  function updateJson() {
    ...
    for (let i = 0; i < inputs.length; i++) {
      ...
      for (let j = 0; j < i; j++) {
        ...
        inputs.forEach((elem) => {
          elem.parentNode.innerHTML = `<p>${elem.value}</p>`
        })
      }
      setEditMode(false)
    }
  ...
  function makeInput(e) {
    ...
    e.target.parentNode.parentNode.childNodes[0].innerHTML = `<input class="input" value=${e.target.parentNode.parentNode.childNodes[0].innerHTML}>`
    e.target.parentNode.parentNode.childNodes[1].innerHTML = `<input class="input" value=${e.target.parentNode.parentNode.childNodes[1].innerHTML}>`
  }
  ...
  const userListTable = (
    <div className="userListDiv">
      <table>
        ...
        <tbody className="tbodyDiv">{userData}</tbody>
      </table>
    </div>
  )
  return (
